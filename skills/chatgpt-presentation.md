...
## Логотип (ОБЯЗАТЕЛЬНО на каждом слайде — чтобы он ТОЧНО отображался)
...
Логотип должен быть в навбаре **на каждом слайде**, слева, высотой 30px. Никогда не пропускай его и не заменяй текстом.

**Как гарантировать, что логотип отобразится (важно для PDF!):**

1. **Для HTML** (пользователь откроет в браузере) — просто используй URL выше в `<img src="...">`. Браузер подгрузит логотип из интернета.
2. **Для PDF** — если ты рендеришь PDF в своей песочнице БЕЗ доступа в интернет, внешняя картинка НЕ загрузится и логотип будет пустым. Чтобы этого не было:
   - Сначала **СКАЧАЙ** логотип в рабочую папку (Python: `urllib.request.urlretrieve(url, "logo-ms.png")`),
   - затем в HTML используй **относительный путь**: `<img src="logo-ms.png" alt="MS">`,
   - либо встрой логотип как **base64 data URI** прямо в `src`.
3. Проверь готовый PDF: логотип MS должен быть виден в левом верхнем углу каждого слайда. Если пусто — значит картинка не загрузилась, скачай её локально и пересобери.
...
/* ===== ПЕЧАТЬ / PDF (размер 16:9 = 1280x720, чтобы текст НЕ был мелким) ===== */
@media print {
...
## ЭКСПОРТ В PDF (КРИТИЧЕСКИ ВАЖНО — иначе текст будет мелким!)

Когда пользователь просит сделать презентацию **в формате PDF** — соблюдай ТОЧНО эти параметры, иначе текст получится мелким:

1. **Размер страницы = 1280×720px** (это стандарт 16:9, как widescreen в PowerPoint = 13.333in × 7.5in).
   - ❌ НЕ A4
   - ❌ НЕ 1920×1080px (это 20 дюймов — текст будет выглядеть крошечным при открытии PDF)
   - ❌ НЕ Letter
2. **Один слайд = одна страница PDF.**
3. **margin: 0** — без полей.
4. **НЕ масштабировать и НЕ «вписывать в страницу»** (отключи fit-to-page / shrink-to-fit / scale).
5. **printBackground: true** — чтобы бордовый акцент, серые блоки и линии печатались.

### Если конвертируешь через headless Chrome / Puppeteer:
```js
await page.pdf({
  width: '1280px',
  height: '720px',
  ...
  pageRanges: '',         // все страницы
  margin: { top: 0, right: 0, bottom: 0, left: 0 }
});
```
Важно: viewport браузера тоже ставь `{ width: 1280, height: 720 }` ПЕРЕД рендером — если поставить больше (1920), текст отрендерится мелким относительно страницы.

### Если конвертируешь другим инструментом:
Передай размер страницы ровно **1280×720px (landscape, 16:9)**, поля 0, фон включён, масштаб 100%.

**Проверка:** открой готовый PDF — текст должен выглядеть ровно так же по размеру, как при открытии HTML в браузере. Если текст мелкий — значит размер страницы слишком большой (1920px), уменьши до 1280×720.

## Как отдавать результат

Отдай HTML-файл целиком, готовый к скачиванию. Пользователь откроет его в браузере — и увидит презентацию в фирменном стиле. Если нужен PDF — экспортируй строго по параметрам выше (1280×720px).
